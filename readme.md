...
1. **工具台首页与路由**
   - 默认路由 `/` 为工具台首页
   - 提供「加解密工具」「日志语句生成」「在线凭证查询」「任务反馈生成」「OSS Key 转换」「流水线监控」「合并监控」等入口
   - `/encrypt` 二级页面可返回首页
   - 页面切换在同一 Tauri 窗口内完成
...
9. **合并监控**
   - 监控云效合并请求 AI 评审完成，系统通知并写入待办（打开详情不删除待办）
   - 配置文件：`yunxiao-merge.json`（与流水线监控独立）
   - 双间隔轮询：列表轮询 + AI 评论轮询；作者白名单按显示名匹配
...
target/release/bundle/nsis/CoterEncrypt_0.1.15_x64-setup.exe
```
